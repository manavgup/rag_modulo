    ...
    def get_document(
        ...
        try:
            ...
            if results:
                ...
                return Document(
                    document_id=document_id, name=document_id, chunks=chunks
                )
            return None
        except MilvusException as e:
            logging.error(
                ...
                chunks_with_scores.append(
                    DocumentChunkWithScore(
                        ...
                        metadata=DocumentChunkMetadata(
                            ...
                            source_id=(
                                hit.entity.get("source_id")
                                if hit.entity.get("source_id")
                                else ""
                            ),
                            url=hit.entity.get("url") if hit.entity.get("url") else "",
                            created_at=(
                                hit.entity.get("created_at")
                                if hit.entity.get("created_at")
                                else ""
                            ),
                            author=(
                                hit.entity.get("author")
                                if hit.entity.get("author")
                                else ""
                            ),
                        ),
                        score=hit.distance,
