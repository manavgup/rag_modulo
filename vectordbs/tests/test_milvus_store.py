...
def milvus_store():
    store = MilvusStore()
    store.create_collection('test_collection', embedding_model_id="sentence-transformers/all-minilm-l6-v2", client=store.client)
    yield store
...
def create_test_documents():
    return [
        Document(document_id="doc1", name="Doc 1", chunks=[
            DocumentChunk(chunk_id="1", text="Hello world", vectors=get_embeddings("Hello world"),
                          metadata=DocumentChunkMetadata(source=Source.WEBSITE, created_at=datetime.now().isoformat() + 'Z'))
        ]),
        Document(document_id="doc2", name="Doc 2", chunks=[
            DocumentChunk(chunk_id="1", text="Hello Jello", vectors=get_embeddings("Hello Jello"),
                          metadata=DocumentChunkMetadata(source=Source.WEBSITE, created_at=datetime.now().isoformat() + 'Z'))
        ]),
        Document(document_id="doc3", name="Doc 3", chunks=[
            DocumentChunk(chunk_id="1", text="Tic Tac Toe", vectors=get_embeddings("Tic Tac Toe"),
                          metadata=DocumentChunkMetadata(source=Source.WEBSITE, created_at=datetime.now().isoformat() + 'Z'))
        ])
    ]

def test_create_collection(milvus_store):
    milvus_store.create_collection('test_new_collection', embedding_model_id="sentence-transformers/all-minilm-l6-v2", client=milvus_store.client)
...
async def test_query_documents(milvus_store):
    ...
    query_result = milvus_store.query('test_collection', QueryWithEmbedding(text="Hello world", vectors=embeddings))
...
async def test_delete_all_documents(milvus_store):
    ...
    with pytest.raises(MilvusException) as excinfo:
        milvus_store.query('test_collection', QueryWithEmbedding(text="Hello world", vectors=get_embeddings("Hello world")))
