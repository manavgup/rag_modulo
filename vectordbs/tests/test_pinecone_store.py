...
def test_create_collection(pinecone_store):
    pinecone_store.create_collection("new-test-index", "sentence-transformers/all-minilm-l6-v2")
...
def test_retrieve_documents_with_string_query(pinecone_store):
    ...
    result = pinecone_store.add_documents(PINECONE_INDEX, documents)
    assert len(result) == 3
    query_results = pinecone_store.retrieve_documents("Hello world", PINECONE_INDEX, limit=10)
    assert query_results is not None
    assert len(query_results) > 0
...
def test_delete_documents(pinecone_store):
    ...
    deleted_count = pinecone_store.delete_documents([chunk_id_to_delete], PINECONE_INDEX)
