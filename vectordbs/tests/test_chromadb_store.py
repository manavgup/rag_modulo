...
from vectordbs.chroma_store import ChromaDBStore  # Adjust the import as per your project structure
...
def test_retrieve_documents_with_string_query(chromadb_store):
    ...
    result = chromadb_store.add_documents(CHROMA_INDEX, documents)
    assert len(result) == 3
    query_results = chromadb_store.retrieve_documents("Hello world", CHROMA_INDEX, limit=10)
    assert query_results is not None
    assert len(query_results) > 0
...
def test_delete_all_documents(chromadb_store):
    ...
    with pytest.raises(ValueError):
        chromadb_store.retrieve_documents(QueryWithEmbedding(text="Hello world", vectors=get_embeddings("Hello world")), CHROMA_INDEX, limit=10)
