...
class ChromaDBStore(VectorStore):
    ...
    def create_collection(
        self, collection_name: str, embedding_model: str = EMBEDDING_MODEL
    ) -> None:
        try:
            if self.client:
    ...
    def query(
        ...
        try:
            ...
            response = self.collection.query(
                query_embeddings=query.vectors, n_results=number_of_results
            )
            ...
            logging.error(
                f"Failed to query ChromaDB collection '{collection_name}': {e}"
            )
            raise
    ...
    def delete_documents(
        ...
        try:
            ...
            logging.info(
                f"Deleted {deleted_count} documents from collection '{collection_name}'"
            )
            return deleted_count
        except Exception as e:
            logging.error(
                f"Failed to delete documents from ChromaDB collection '{collection_name}': {e}"
            )
            return 0

    def _convert_to_chunk(
        self, id: str, text: str, vectors: Optional[List[float]], metadata: Dict
    ) -> DocumentChunk:
        return DocumentChunk(
            ...
            metadata=DocumentChunkMetadata(
                source=(
                    Source(metadata["source"]) if metadata["source"] else Source.OTHER
                ),
    ...
    def _process_search_results(self, response: Dict) -> List[QueryResult]:
        ...
        for i in range(len(ids)):
            ...
            results.append(
                QueryResult(data=[chunk], similarities=[distances[i]], ids=[ids[i]])
            )
        return results
    ...
    def _build_filters(
        ...
        if not filter:
            return {}
        return {filter.field_name: filter.value}
