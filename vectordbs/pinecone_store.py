...
class PineconeStore(VectorStore):
    ...
    def create_collection(self, collection_name: str, embedding_model: str = "sentence-transformers/all-minilm-l6-v2") -> None:
        try: 
            if collection_name in self.client.list_indexes().names():
                ...
                self.client.create_index(name=collection_name, 
                                            dimension=384, 
                                            metric="cosine", 
                                            spec=ServerlessSpec(
    ...
    def _process_search_results(self, response: Dict) -> List[QueryResult]:
        ...
        for match in response['matches']:
            ...
            results.append(QueryResult(data=[chunk], similarities=[match['score']], ids=[match['id']]))
        return results
