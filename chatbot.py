...
docs = []
uploaded_files = []
creds = {"url": api_endpoint, "apikey": api_key}
...
def index_with_chroma(docs, EMBED, index_name):
    chroma_db = Chroma.from_documents(
        docs, EMBED, collection_name=index_name, persist_directory=CHROMA_DB_PATH
    )
    chroma_db.persist()
...
def index_documents(
    ...
    for uploaded_file in uploaded_files:
        ...
        with tempfile.NamedTemporaryFile(
            delete=False, suffix=random_suffix
        ) as temp_file:
            ...
            text_splitter = RecursiveCharacterTextSplitter(
                chunk_size=chunk_size, chunk_overlap=chunk_size, separators=["\n"]
            )
...
with st.sidebar:
    model = st.selectbox(
        label="Model", options=model_ids, index=model_ids.index("google/flan-ul2")
    )
    st.session_state["llm"] = model

    selected_option = st.sidebar.selectbox(
        "Choose a vector store",
        ["chroma", "milvus", "pinecone", "elasticsearch"],
        key="vectordb",
    )
...
with st.sidebar.expander("Advanced Options", expanded=False):
    ...
    index_name = st.text_input("Index name", value="demo")
    ...
    chain_type_help_root = "https://python.langchain.com/docs/modules/chains/document/"
    chain_type_help = "\n".join(
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            llm = LangChainInterface(
                model=st.session_state["llm"], params=params, credentials=creds
            )
            EMBED = HuggingFaceEmbeddings(model_name=selected_embedding)
