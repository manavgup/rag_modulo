...
def semantic_chunking(text: str, min_chunk_size: int = 1, max_chunk_size: int = 100, threshold: float = 0.8) -> List[str]:
    """
    ...
    for i, (sentence, embedding) in enumerate(zip(sentences, embeddings_array)):
        ...
        logger.info(f"Similarity: {similarity}", f"** Current chunk length: {len(current_chunk)}")

        if similarity >= threshold and len(current_chunk) < max_chunk_size:
            # Add to current chunk
            current_chunk.append(sentence)
            # Update current embedding as the average
            current_embedding = np.mean(np.vstack([current_embedding, embedding]), axis=0)
        else:
...
def semantic_chunking_for_tables(tables: List[List[List[str]]],
    ...
    for table in tables:
        ...
        for row in table_rows:
            row_chunks = semantic_chunking(row, min_chunk_size, max_chunk_size, threshold)
...
def get_chunking_method():
    if settings.chunking_strategy.lower() == "semantic":
        return lambda text: semantic_chunking(text, settings.min_chunk_size, settings.max_chunk_size, settings.semantic_threshold)
    else:
        return lambda text: simple_chunking(text, settings.min_chunk_size, settings.max_chunk_size, settings.chunk_overlap)
