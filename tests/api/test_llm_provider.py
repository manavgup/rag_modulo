...
class TestLLMProvider(BaseTestRouter):
    ...
    async def test_update_provider(self, ensure_watsonx_provider):
        ...
        update_data = {
            "base_url": "https://updated-api.test.com",
            "project_id": f"updated-project-{uuid4()}"
        }
        response = await self.put(
            f"/api/llm-providers/{ensure_watsonx_provider.id}",
            json=update_data
        )
        self.assert_success(response)
        data = response.json()
    ...
    async def test_get_provider_models(self, ensure_watsonx_provider):
        ...
        response = await self.get(
            f"/api/llm-providers/{ensure_watsonx_provider.id}/models"
        )
        self.assert_success(response)
    ...
    async def test_get_models_by_type(self):
        ...
        response = await self.get(
            "/api/llm-providers/models/type/GENERATION"
        )
        self.assert_success(response)
        models = response.json()
    ...
    async def test_update_model(self, test_model_data):
        ...
        response = await self.put(
            f"/api/llm-providers/models/{model_id}",
            json=update_data
        )
        self.assert_success(response)
        data = response.json()
