"""
Integration tests for PipelineService - testing with real database.
...
class TestPipelineService:
    """Integration tests for PipelineService with real database."""

    @pytest.fixture
    def pipeline_service(self, real_db_session: Session) -> PipelineService:
        """Create a real PipelineService with real database connection using transaction rollback."""
        settings = get_settings()
        return PipelineService(real_db_session, settings)
    ...
    def test_get_user_pipelines_with_invalid_user_id(self, pipeline_service: PipelineService):
        ...
        try:
            ...
            # If it raises an error, it should be about configuration/retrieval failure
            error_message = str(exc).lower()
            assert any(keyword in error_message for keyword in ["not found", "user", "404", "failed", "configuration", "retrieve"])
