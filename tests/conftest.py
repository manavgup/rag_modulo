...
3. Test database isolation (separate test databases)
4. General test configuration
"""

# ============================================================================
# CRITICAL: pytest_configure hook runs BEFORE anything else
# ============================================================================
def pytest_configure(config):
    """Configure pytest and set environment variables before any imports."""
    import os
    import sys

    # Determine test type from pytest invocation
    ...
    # Set environment variables BEFORE any backend code is imported
    if test_type == "e2e":
        os.environ["MILVUS_COLLECTION_PREFIX"] = "e2e_"
        # Enable mock authentication for E2E tests (bypasses OIDC)
        os.environ["SKIP_AUTH"] = "true"
        # Ensure localhost is used for database connections (not Docker network name "postgres")
        os.environ["COLLECTIONDB_HOST"] = "localhost"
    elif test_type == "integration":
        os.environ["MILVUS_COLLECTION_PREFIX"] = "test_"

# ============================================================================
# Set environment variables at module import time as well (belt and suspenders)
# ============================================================================
import os
import sys

# Determine test type from pytest marks
test_type = "integration"  # default
for arg in sys.argv:
    if "e2e" in arg:
        test_type = "e2e"
...
# Set collection prefixes based on test type for Milvus isolation
# Database isolation is handled by transaction rollback (no separate DB needed)
if test_type == "e2e":
    os.environ.setdefault("MILVUS_COLLECTION_PREFIX", "e2e_")
    # Enable mock authentication for E2E tests (bypasses OIDC)
    os.environ.setdefault("SKIP_AUTH", "true")
    # Ensure localhost is used for database connections (not Docker network name "postgres")
    os.environ.setdefault("COLLECTIONDB_HOST", "localhost")
elif test_type == "integration":
    os.environ.setdefault("MILVUS_COLLECTION_PREFIX", "test_")
# Unit tests don't need Milvus

# ============================================================================
# Now safe to import backend modules (env vars are set)
# ============================================================================
...
# ============================================================================
# Test isolation is achieved through transaction rollback (not separate databases)
# - Unit tests: Use mocked dependencies (no real DB)
# - Integration tests: Use transaction rollback on rag_modulo DB + test_ Milvus prefix
# - E2E tests: Use transaction rollback on rag_modulo DB + e2e_ Milvus prefix
#
# Transaction rollback ensures:
# - Each test runs in an isolated transaction
# - All changes are rolled back after test completion
# - No data pollution between tests or with dev environment

# Log test type configuration
logger.info(f"Test type: {test_type}, Collection prefix: {os.environ.get('MILVUS_COLLECTION_PREFIX', 'none')}, SKIP_AUTH: {os.environ.get('SKIP_AUTH', 'false')}")
...
def e2e_settings():
    ...
    Ensures correct database host for local E2E testing (localhost not postgres).
    """
