...
class VectorStore(ABC):
    ...
    def health_check(self, timeout: float = 5.0) -> HealthCheckResponse:
        ...
        try:
            ...
            return VectorDBResponse.success_response(
                data=health_data, message="Health check passed", metadata={"elapsed_seconds": elapsed}
            )
        except Exception as e:
            elapsed = time.time() - start_time
            logger.error("Health check failed: %s", str(e))
            return VectorDBResponse.error_response(
    ...
    def get_collection_stats(self, collection_name: str) -> VectorDBResponse:
        ...
        try:
            ...
            return VectorDBResponse.success_response(data=stats, message=f"Statistics for {collection_name}")
        except Exception as e:
            logger.error("Failed to get stats for collection %s: %s", collection_name, str(e))
            return VectorDBResponse.error_response(error=str(e), metadata={"collection_name": collection_name})
