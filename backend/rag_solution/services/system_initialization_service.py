...
class SystemInitializationService:
    ...
    def initialize_providers(self, raise_on_error: bool = False) -> List[LLMProviderOutput]:
        try:
            existing_providers = {
                p.name: p for p in self.llm_provider_service.get_all_providers()
            }
        ...
        for name, config in default_configs.items():
            try:
                provider = self._initialize_single_provider(
                    name, 
                    config, 
    ...
    def _initialize_single_provider(
        ...
        existing_provider: Optional[LLMProviderOutput],
        raise_on_error: bool
    ) -> Optional[LLMProviderOutput]:
        try:
            ...
            logger.error(f"Provider initialization error: {str(e)}")
            if raise_on_error:
                raise
            return None
    ...
    def _setup_watsonx_models(self, provider_id: UUID, raise_on_error: bool):
        try:
            ...
            logger.error(f"Error creating WatsonX models: {str(e)}")
            if raise_on_error:
                raise