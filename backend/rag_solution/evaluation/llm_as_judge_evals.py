...
def get_schema(pydantic_object:  pydantic.BaseModel,empty:bool=False,json_output:bool=False) -> Union[str,Dict]:
    ...
    def batch_evaluate(self, inputs: List):
        ...
        for generated_text in result:
            try:
                ...
                logger.error(
                    f"Output Parser Exception during batch processing for [{generated_text}] : {ex}"
                )
                final_json = get_schema(self.pydantic_model,json_output=True,empty=True)
...
class AnswerRelevanceEvaluator(BaseEvaluator):
    ...
    def batch_evaluate(self, inputs: List):
        result = super().batch_evaluate(inputs)
        for item in result:
            item["answer_relevance_score"] = self.scores.get(
                item.get("answer_relevance_rate", 0)
            )
        return result
...
class AnswerSimilarityEvaluator(BaseEvaluator):
    ...
    def evaluate(
        self, question: str, answer: str, reference_answer: str, show_logs: bool = False
    ):
