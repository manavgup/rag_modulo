...
Examples:
  ...
  rag-cli health check --api --database
    ...
    parser.add_argument("--output", "-o", choices=["table", "json", "yaml"], default="table", help="Output format (default: table)")
...
def _add_auth_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    """
    auth_parser = subparsers.add_parser("auth", help="Authentication management", description="Manage authentication and profiles")
    ...
    login_parser.add_argument("--provider", choices=["local", "ibm"], default="local", help="Authentication provider (default: local)")
    ...
    token_parser.add_argument("--expires-in", type=int, default=86400, help="Token expiration in seconds (default: 86400/24 hours)")
...
def _add_collection_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    Args:
        subparsers: Subparsers action from main parser
    """
    collections_parser = subparsers.add_parser("collections", help="Collection management", description="Manage document collections")
    ...
    share_collection_parser.add_argument("--permission", default="read", choices=["read", "write"], help="Permission level")
...
def _add_document_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    Args:
        subparsers: Subparsers action from main parser
    """
    documents_parser = subparsers.add_parser("documents", help="Document management", description="Manage documents in collections")
...
def _add_search_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    Args:
        subparsers: Subparsers action from main parser
    """
    search_parser = subparsers.add_parser("search", help="Search operations", description="Search documents and collections")
...
def _add_health_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    Args:
        subparsers: Subparsers action from main parser
    """
    health_parser = subparsers.add_parser("health", help="System health checks", description="Check system component health")
...
def _add_config_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    Args:
        subparsers: Subparsers action from main parser
    """
    config_parser = subparsers.add_parser("config", help="Configuration management", description="Manage CLI configuration")
        ...
        if parsed_args.command == "auth":
            ...
            if hasattr(parsed_args, "auth_command") and parsed_args.auth_command:
                if parsed_args.auth_command == "login":
                    ...
                    result = auth_cmd.login(username=username, password=password, provider=provider, interactive=interactive)
                ...
                return CLIResult(exit_code=1, error="No auth command specified. Use: login, set-token, logout, or status")
            ...
            if hasattr(parsed_args, "collections_command") and parsed_args.collections_command:
                if parsed_args.collections_command == "list":
                    ...
                    result = collections_cmd.get_collection(collection_id=parsed_args.collection_id, include_stats=getattr(parsed_args, "stats", False))
                else:
                    return CLIResult(exit_code=1, error=f"Unknown collections command: {parsed_args.collections_command}")
