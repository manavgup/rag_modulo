...
from backend.rag_solution.generation.generator import Generator, HuggingFaceGenerator, WatsonxGenerator

class TestGenerator(unittest.TestCase):

    def test_huggingface_generator(self):
        config = {'type': 'huggingface', 'model_name': 'gpt2'}
        with patch('backend.rag_solution.generation.generator.pipeline') as mock_pipeline:
    ...
    def test_watsonx_generator(self):
        config = {'type': 'watsonx'}
        with patch('backend.rag_solution.generation.generator.generate_text') as mock_generate_text:
