...
class TestConversationIntegrationTDD:
    ...
    def test_conversation_flow_with_context_management(
        ...
        session_input = ConversationSessionInput(
            user_id=user_id, collection_id=collection_id, session_name="Test Conversation"
        )
        ...
        # Get context
        context = context_manager_service.build_context_from_messages(session.id, [response1, response2])
    ...
    @pytest.mark.integration
    def test_conversation_with_search_integration(
        ...
        # Create session
        session_input = ConversationSessionInput(
            user_id=user_id, collection_id=collection_id, session_name="Search Integration Test"
        )
        session = conversation_service.create_session(session_input)
        ...
        search_input = SearchInput(
            question="What documents do we have about AI?", collection_id=collection_id, user_id=user_id
        )
    ...
    def test_conversation_session_lifecycle(self, conversation_service: ConversationService) -> None:
        ...
        # Create session
        session_input = ConversationSessionInput(
            user_id=user_id, collection_id=collection_id, session_name="Lifecycle Test"
        )
        session = conversation_service.create_session(session_input)
        assert session.status == SessionStatus.ACTIVE
