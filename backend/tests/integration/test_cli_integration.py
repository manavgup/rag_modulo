...
class TestCLILeveragesExistingServices:
    ...
    def test_cli_uses_existing_collection_endpoints(self):
        ...
        with patch.object(api_client, "post") as mock_post:
            ...
            # Should use same endpoint as existing API tests
            mock_post.assert_called_once_with("/api/collections", data={"name": "Test", "description": "Description"})
    ...
    def test_cli_uses_existing_search_endpoints(self):
        ...
        with patch.object(api_client, "post") as mock_post:
            mock_post.return_value = {"answer": "Test answer", "retrieved_chunks": [], "confidence_score": 0.8}

            commands.query("collection123", "test query")

            # Should match existing search service endpoints
            mock_post.assert_called_once_with("/api/search/query", data={"collection_id": "collection123", "query": "test query"})
...
class TestCLIConfigurationManagement:
    ...
    def test_profile_management_integration(self, temp_profile_dir):
        ...
        with patch("rag_solution.cli.auth.ProfileManager.profiles_dir", temp_profile_dir):
            # Create profile
            create_result = main_cli(["auth", "profiles", "create", "test-profile", "--api-url", "https://test.example.com", "--description", "Test profile"])
    ...
    def test_configuration_validation_integration(self):
        ...
        valid_result = main_cli(["config", "validate", "--api-url", "https://valid.example.com", "--timeout", "30"])

        assert valid_result.exit_code == 0

        # Test invalid configuration
        invalid_result = main_cli(["config", "validate", "--api-url", "invalid-url", "--timeout", "-1"])
