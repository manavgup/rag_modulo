...
def test_complete_configuration_flow(db_session: Session, base_user: Any) -> None:
...
def test_update_template_flow(db_session: Session, base_user: Any) -> None:
    ...
    updated_template = template_service.create_template(
        PromptTemplateInput(
            name="test-template-updated",  # Different name for new template
            user_id=base_user.id,
            template_type=PromptTemplateType.RAG_QUERY,
    ...
    assert updated_template.id != template.id  # Different ID means it was created as new template
    ...
    assert updated_template.example_inputs is not None
