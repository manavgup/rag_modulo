...
class TestSearchDataValidation:
    ...
    TEST_COLLECTIONS = {
        "machine_learning": {
            ...
            "languages": ["en"],
            "quality_metrics": {"min_document_length": 1000, "max_document_length": 50000, "avg_document_length": 10000},
        },
        "python_programming": {
            ...
            "languages": ["en"],
            "quality_metrics": {"min_document_length": 500, "max_document_length": 30000, "avg_document_length": 8000},
        },
        "data_science": {
            ...
            "languages": ["en"],
            "quality_metrics": {"min_document_length": 2000, "max_document_length": 40000, "avg_document_length": 12000},
        },
        "artificial_intelligence": {
            ...
            "languages": ["en"],
            "quality_metrics": {"min_document_length": 1500, "max_document_length": 45000, "avg_document_length": 15000},
        },
    }
    ...
    PIPELINE_CONFIGURATIONS = {
        "default": {
            ...
            "expected_performance": {"response_time_ms": 2000, "accuracy_threshold": 0.85},
        },
        "fast": {
            ...
            "llm_provider": "watsonx",
            "expected_performance": {"response_time_ms": 1000, "accuracy_threshold": 0.80},
        },
        "comprehensive": {
            ...
            "context_strategy": "weighted",
            "llm_provider": "watsonx",
            "expected_performance": {"response_time_ms": 4000, "accuracy_threshold": 0.90},
        },
    }
    ...
    def test_factual_query_validation(self, test_user_id: UUID, mock_search_service: Mock):
        ...
        for query_data in factual_queries:
            if query_data["question"]:  # Skip empty queries
                ...
                assert len(result.documents) >= query_data["expected_sources"], f"Expected at least {query_data['expected_sources']} sources for query: {query_data['question']}"
                assert len(result.query_results) >= query_data["expected_sources"], f"Expected at least {query_data['expected_sources']} query results for query: {query_data['question']}"
    ...
    def test_analytical_query_validation(self, test_user_id: UUID, mock_search_service: Mock):
        ...
        for query_data in analytical_queries:
            ...
            # Assert
            assert result is not None
            assert len(result.documents) >= query_data["expected_sources"], f"Expected at least {query_data['expected_sources']} sources for analytical query: {query_data['question']}"

            # Verify answer contains expected keywords
            answer_lower = result.answer.lower()
            for keyword in query_data["expected_keywords"]:
                assert keyword.lower() in answer_lower, f"Expected keyword '{keyword}' not found in answer for query: {query_data['question']}"
    ...
    def test_technical_query_validation(self, test_user_id: UUID, mock_search_service: Mock):
        ...
        for query_data in technical_queries:
            ...
            # Assert
            assert result is not None
            assert len(result.documents) >= query_data["expected_sources"], f"Expected at least {query_data['expected_sources']} sources for technical query: {query_data['question']}"

            # Verify technical depth
            assert result.evaluation is not None
            assert "technical_depth" in result.evaluation
            assert result.evaluation["technical_depth"] >= 0.8, f"Technical depth should be high for technical query: {query_data['question']}"
    ...
    def test_practical_query_validation(self, test_user_id: UUID, mock_search_service: Mock):
        ...
        for query_data in practical_queries:
            ...
            # Assert
            assert result is not None
            assert len(result.documents) >= query_data["expected_sources"], f"Expected at least {query_data['expected_sources']} sources for practical query: {query_data['question']}"
        ...
        for query_data in edge_case_queries:
            ...
            if query_data["expected_answer_type"] == "error":
                ...
                if query_data["expected_sources"] > 0:
                    assert len(result.documents) >= query_data["expected_sources"], f"Expected at least {query_data['expected_sources']} sources for edge case query"
