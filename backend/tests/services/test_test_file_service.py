...
@pytest.fixture
def collection(collection_service: CollectionService, user_service: UserService) -> Any:
...
def mock_metadata() -> FileMetadata:
    return FileMetadata(
        ...
        author="John Doe",
        subject="Testing",
        keywords={"tags": "test,file,metadata"},
        creator="Test Suite",
        producer="Pytest",
        creation_date=datetime(2023, 8, 18),
        mod_date=datetime(2023, 8, 18),
        total_pages=1,
    )
...
def test_get_file_by_id(file_management_service: FileManagementService, collection: Any, mock_upload_file: UploadFile) -> None:
    file_record = file_management_service.upload_and_create_file_record(mock_upload_file, collection.user_ids[0], collection.id, "test_doc_id")
...
def test_get_file_by_name(file_management_service: FileManagementService, collection: Any, mock_upload_file: UploadFile) -> None:
...
def test_update_file(
    ...
    file_record = file_management_service.upload_and_create_file_record(mock_upload_file, collection.user_ids[0], collection.id, "test_doc_id", metadata=mock_metadata)
...
def test_get_files(file_management_service: FileManagementService, collection: Any, mock_upload_file: UploadFile) -> None:
    file_management_service.upload_and_create_file_record(mock_upload_file, collection.user_ids[0], collection.id, "test_document_id")
...
def test_get_file_path(file_management_service: FileManagementService, collection: Any, mock_upload_file: UploadFile) -> None:
    file_management_service.upload_and_create_file_record(mock_upload_file, collection.user_ids[0], collection.id, "test_document_id")
...
def test_delete_file(file_management_service: FileManagementService, collection: Any, mock_upload_file: UploadFile) -> None:
    file_record = file_management_service.upload_and_create_file_record(mock_upload_file, collection.user_ids[0], collection.id, "test_doc_id")
...
def test_delete_files(file_management_service: FileManagementService, collection: Any, mock_upload_file: UploadFile) -> None:
    file_management_service.upload_and_create_file_record(mock_upload_file, collection.user_ids[0], collection.id, "test_document_id")
...
def test_get_files_by_collection(file_management_service: FileManagementService, collection: Any, mock_upload_file: UploadFile) -> None:
    file_management_service.upload_and_create_file_record(mock_upload_file, collection.user_ids[0], collection.id, "test_document_id")
...
def test_update_file_metadata(
    ...
    updated_metadata = FileMetadata(
        ...
        keywords={"tags": "updated,test,file,metadata"},
        creator="Test Suite",
        producer="Pytest",
        creation_date=datetime(2023, 8, 18),
        mod_date=datetime(2023, 8, 19),
        total_pages=2,
    )
...
def test_get_files_by_collection_with_metadata(
    ...
    file_management_service.upload_and_create_file_record(mock_upload_file, collection.user_ids[0], collection.id, "test_doc_id", metadata=mock_metadata)
    files = file_management_service.get_files_by_collection(collection.id)
    assert len(files) == 1
    assert isinstance(files[0], FileOutput)
    assert files[0].filename == "test_file.txt"
