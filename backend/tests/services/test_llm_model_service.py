...
def test_get_models_by_provider(
    ...
    model2 = llm_model_service.create_model(base_model_input.model_copy(
...
def test_get_models_by_type(
    ...
    gen_model = llm_model_service.create_model(base_model_input.model_copy(
        update={"model_type": ModelType.GENERATION}
    ))
    embed_model = llm_model_service.create_model(base_model_input.model_copy(
        update={
...
def test_set_default_model(
    ...
    # Create two models
    model1 = llm_model_service.create_model(base_model_input.model_copy(
        update={"is_default": False}
    ))
    model2 = llm_model_service.create_model(base_model_input.model_copy(
        update={"model_id": "model2", "is_default": False}
    ))
...
def test_get_default_model(
    ...
    model = llm_model_service.create_model(base_model_input.model_copy(
        update={"is_default": True}
    ))
    
    # Get default model
    default = llm_model_service.get_default_model(
        provider.id,
        base_model_input.model_type
    )
...
# -------------------------------------------
def test_model_runtime_settings(
    llm_model_service,
    base_model_input,
    ensure_watsonx_provider
):
...
def test_model_type_specific_settings(
    llm_model_service,
    base_model_input,
    ensure_watsonx_provider
):
...
def test_multiple_models_same_provider(
    ...
    models = []
    for i in range(3):
        model_input = base_model_input.model_copy(update={
            "model_id": f"model-{i}",
