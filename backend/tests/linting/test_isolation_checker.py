...
class TestIsolationChecker:
    ...
    def _check_test_function(self, node: ast.FunctionDef, filepath: str) -> None:
        """Check if a test function violates isolation principles."""
        # Check if this is an atomic test
        is_atomic = any((isinstance(dec, ast.Name) and dec.id == "atomic") or (isinstance(dec, ast.Attribute) and dec.attr == "atomic") for dec in node.decorator_list)
    ...
    def _check_function_call(self, node: ast.Call, test_func: ast.FunctionDef, _filepath: str) -> None:
    ...
    def _check_attribute_access(self, node: ast.Attribute, test_func: ast.FunctionDef, _filepath: str) -> None:
        """Check attribute access in atomic tests."""
        # Check for settings access without mocking
        if isinstance(node.value, ast.Name) and node.value.id == "settings" and not self._is_in_patch_context(node, test_func):
    ...
    def _check_import(self, node: ast.ImportFrom, filepath: str) -> None:
        """Check imports for problematic patterns."""
        # Only check files that contain atomic tests
        if node.module == "core.config" and any(alias.name == "settings" for alias in node.names) and "test" in filepath.lower() and self._has_atomic_tests(filepath):
            self.violations.append((node.lineno, node.col_offset, "Test imports global settings - use fixtures or mocking instead"))

    def _check_assignment(self, _node: ast.Assign, _filepath: str) -> None:
    ...
    def _is_in_patch_context(self, _node: ast.Attribute, test_func: ast.FunctionDef) -> bool:
        ...
        return any(isinstance(dec, ast.Call) and isinstance(dec.func, ast.Attribute) and dec.func.attr == "patch" for dec in test_func.decorator_list)
