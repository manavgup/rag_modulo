...
def test_create_collection(test_client, test_collection, base_user, auth_headers):
    ...
    response = test_client.post("/api/collections", json=collection_data, headers=auth_headers)
...
def test_get_collection(test_client, collection_service, test_collection, auth_headers):
    ...
    response = test_client.get(f"/api/collections/{collection.id}", headers=auth_headers)
...
def test_delete_collection(test_client, collection_service, test_collection, auth_headers):
    ...
    response = test_client.delete(f"/api/collections/{collection.id}", headers=auth_headers)
    assert response.status_code == 200

    # Verify deletion
    get_response = test_client.get(f"/api/collections/{collection.id}", headers=auth_headers)
...
def test_llm_parameters_crud(
    ...
    # Get LLM parameters
    get_response = test_client.get(f"/api/users/{base_user.id}/llm-parameters", headers=auth_headers)
...
def test_file_operations(test_client, collection_service, test_collection, base_user, auth_headers):
    ...
    delete_response = test_client.delete(f"/api/users/{base_user.id}/files/{file_id}", headers=auth_headers)
...
def test_question_operations(test_client, collection_service, test_collection, base_user, auth_headers):
    ...
    # Get questions
    get_response = test_client.get(f"/api/collections/{collection.id}/questions", headers=auth_headers)
    assert get_response.status_code == 200
    ...
    # Delete questions
    delete_response = test_client.delete(f"/api/collections/{collection.id}/questions", headers=auth_headers)
...
def test_validation_errors(test_client, auth_headers):
    ...
    response = test_client.get("/api/collections/not-a-valid-uuid", headers=auth_headers)
    assert response.status_code == 422
...
def test_not_found_errors(test_client, auth_headers):
    ...
    response = test_client.get(f"/api/collections/{non_existent_id}", headers=auth_headers)
    ...
    response = test_client.get(f"/api/collections/{non_existent_id}/llm-parameters", headers=auth_headers)
    assert response.status_code == 404, f"Expected 404, got {response.status_code}. Response: {response.json()}"

    # Test non-existent questions
    response = test_client.get(f"/api/collections/{non_existent_id}/questions", headers=auth_headers)
    assert response.status_code == 404, f"Expected 404, got {response.status_code}. Response: {response.json()}"
