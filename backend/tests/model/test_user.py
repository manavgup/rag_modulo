...
def user_service(db_session: Session) -> UserService:
    # Assuming the UserService constructor only requires the database session
...
def test_get_user(user_service: UserService, user_input: UserInput) -> None:
...
def test_update_user(user_service: UserService, user_input: UserInput) -> None:
    created_user = user_service.create_user(user_input)
...
def test_delete_user(user_service: UserService, user_input: UserInput) -> None:
    created_user = user_service.create_user(user_input)
...
def test_get_user_by_ibm_id(user_service: UserService, user_input: UserInput) -> None:
    created_user = user_service.create_user(user_input)
...
def test_get_non_existent_user_by_ibm_id(user_service: UserService) -> None:
    with pytest.raises(HTTPException) as exc_info:
        user_service.get_user_by_ibm_id("non_existent_ibm_id")
    assert exc_info.value.status_code == 404


def test_update_non_existent_user(user_service: UserService, user_input: UserInput) -> None:
    with pytest.raises(HTTPException) as exc_info:
        user_service.update_user(UUID4("00000000-0000-0000-0000-000000000000"), user_input)
    assert exc_info.value.status_code == 404


def test_delete_non_existent_user(user_service: UserService) -> None:
    with pytest.raises(HTTPException) as exc_info:
...
def test_create_user_with_invalid_input(user_service: UserService, invalid_input: Any, error_message: str) -> None:
    with pytest.raises(ValidationError) as exc_info:
        UserInput(**invalid_input)
    assert error_message in str(exc_info.value)


def test_create_duplicate_user(user_service: UserService, user_input: UserInput) -> None:
...
def test_update_user_email(user_service: UserService, user_input: UserInput) -> None:
    created_user = user_service.create_user(user_input)
...
def test_update_user_name(user_service: UserService, user_input: UserInput) -> None:
    created_user = user_service.create_user(user_input)
