...
class TestUserCollections:
    ...
    def test_get_user_collections_unauthorized(self, client: TestClient, test_user: User):
        """Test user collections retrieval without authorization."""
        response = client.get(f"/api/users/{test_user.id}/collections")
        assert response.status_code == 403

    def test_add_user_to_collection_success(self, client: TestClient, test_user: User, test_collection: Collection):
...
class TestUserTeams:
    ...
    def test_add_user_to_team_success(self, client: TestClient, test_user: User, test_team: Team):
...
class TestUserFiles:
    def test_upload_file_success(self, client: TestClient, test_user: User, test_collection: Collection):
    ...
    def test_upload_file_with_metadata_success(self, client: TestClient, test_user: User, test_collection: Collection):
...
class TestUserPipelines:
    ...
    def test_create_pipeline_success(self, client: TestClient, test_user: User):
    ...
    def test_update_pipeline_success(self, client: TestClient, test_user: User, test_pipeline: PipelineConfig):
        ...
        response = client.put(f"/api/users/{test_user.id}/pipelines/{test_pipeline.id}", json=update_data)
        assert response.status_code == 200
        assert response.json()["name"] == "Updated Pipeline"

    def test_delete_pipeline_success(self, client: TestClient, test_user: User, test_pipeline: PipelineConfig):
        """Test successful pipeline deletion."""
        # Mock authentication
        client.headers = {"Authorization": f"Bearer {test_user.id}"}

        response = client.delete(f"/api/users/{test_user.id}/pipelines/{test_pipeline.id}")
        assert response.status_code == 200
        assert response.json()["status"] == "success"

    def test_set_default_pipeline_success(self, client: TestClient, test_user: User, test_pipeline: PipelineConfig):
        """Test successful setting of default pipeline."""
        # Mock authentication
        client.headers = {"Authorization": f"Bearer {test_user.id}"}

        response = client.put(f"/api/users/{test_user.id}/pipelines/{test_pipeline.id}/default")
        assert response.status_code == 200
        assert response.json()["is_default"] is True

    def test_validate_pipeline_success(self, client: TestClient, test_user: User, test_pipeline: PipelineConfig):
        """Test successful pipeline validation."""
        # Mock authentication
        client.headers = {"Authorization": f"Bearer {test_user.id}"}

        response = client.post(f"/api/users/{test_user.id}/pipelines/{test_pipeline.id}/validate")
        assert response.status_code == 200
        assert "result" in response.json()

    def test_test_pipeline_success(self, client: TestClient, test_user: User, test_pipeline: PipelineConfig):
        """Test successful pipeline testing."""
        # Mock authentication
        client.headers = {"Authorization": f"Bearer {test_user.id}"}

        response = client.post(
            f"/api/users/{test_user.id}/pipelines/{test_pipeline.id}/test", params={"query": "What is this test about?"}
        )
        assert response.status_code == 200
