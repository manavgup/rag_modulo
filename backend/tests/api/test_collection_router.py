...
def test_create_collection_with_documents(test_client: TestClient, auth_headers: dict, base_user, test_documents):
    ...
    response = test_client.post(
        ...
        data={"collection_name": "Test Collection with Files", "is_private": "false", "user_id": str(base_user.id)},
        files=files,
...
def test_get_file_path(test_client: TestClient, auth_headers: dict, base_collection):
    ...
    # Then get its path
    response = test_client.get(f"/api/collections/{base_collection.id}/files/{file_id}", headers=auth_headers)
