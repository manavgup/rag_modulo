...
from rag_solution.cli.exceptions import RAGCLIError, AuthenticationError, ValidationError
...
class TestCLICommandParsing:
    ...
    def test_main_commands_parsing(self):
        ...
        # Test users command
        args = parser.parse_args(['users', 'create', 'test@example.com'])
...
class TestCLIConfiguration:
    ...
    def test_config_validation(self):
        ...
        with pytest.raises(ValidationError):
            invalid_config = RAGConfig(api_url='invalid-url')
            invalid_config.validate()
            
        # Invalid timeout
        with pytest.raises(ValidationError):
            invalid_config = RAGConfig(timeout=-1)
            invalid_config.validate()
...
class TestCLIOutputFormatting:
    ...
    def test_format_table_output(self):
        ...
        data = [
            {'name': 'Collection 1', 'status': 'active'},
            {'name': 'Collection 2', 'status': 'processing'}
        ]
