...
class TestRetrieverFactory(unittest.TestCase):
    ...
    def test_create_hybrid_retriever(self) -> None:
        ...
        # Cast to HybridRetriever to access vector_weight attribute
        hybrid_retriever = retriever
        assert isinstance(hybrid_retriever, HybridRetriever)
        self.assertEqual(hybrid_retriever.vector_weight, 0.8)

    def test_create_vector_retriever(self) -> None:
        config = {"type": "vector"}
        retriever = RetrieverFactory.create_retriever(config, self.mock_document_store)

        self.assertIsInstance(retriever, VectorRetriever)

    def test_create_keyword_retriever(self) -> None:
        config = {"type": "keyword"}
        retriever = RetrieverFactory.create_retriever(config, self.mock_document_store)

        self.assertIsInstance(retriever, KeywordRetriever)

    def test_default_to_hybrid_retriever(self) -> None:
        config: dict[str, Any] = {}  # Empty config should default to hybrid
        retriever = RetrieverFactory.create_retriever(config, self.mock_document_store)
        ...
        hybrid_retriever = retriever
        assert isinstance(hybrid_retriever, HybridRetriever)
        self.assertEqual(hybrid_retriever.vector_weight, 0.7)  # Default weight

    def test_invalid_retriever_type(self) -> None:
