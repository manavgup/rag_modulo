...
def test_team(db_session: Session, test_team_input: TeamInput) -> Team:
    ...
    team = Team(
        name=test_team_input.name, description=test_team_input.description, is_private=test_team_input.is_private
    )
    db_session.add(team)
    db_session.commit()
...
@pytest.mark.atomic
def test_create_team_success(db_session: Session, test_team_input: TeamInput):
...
@pytest.mark.atomic
def test_create_team_duplicate_name(db_session: Session, test_team: Team, test_team_input: TeamInput):
...
@pytest.mark.atomic
def test_get_team_by_id_success(db_session: Session, test_team: Team):
...
@pytest.mark.atomic
def test_update_team_success(db_session: Session, test_team: Team):
...
@pytest.mark.atomic
def test_update_team_not_found(db_session: Session, test_team_input: TeamInput):
...
@pytest.mark.atomic
def test_delete_team_success(db_session: Session, test_team: Team):
...
@pytest.mark.atomic
def test_get_team_users(db_session: Session, test_team: Team, base_user: User):
...
@pytest.mark.atomic
def test_add_user_to_team(db_session: Session, test_team: Team, base_user: User):
...
@pytest.mark.atomic
def test_remove_user_from_team(db_session: Session, test_team: Team, base_user: User):
...
@pytest.mark.atomic
def test_list_teams(db_session: Session, test_team: Team):
    """Test listing teams."""
    service = TeamService(db_session)
...
@pytest.mark.atomic
def test_list_teams_pagination(db_session: Session, test_team: Team):
    """Test teams listing with pagination."""
    service = TeamService(db_session)

    # Create additional teams
    for i in range(5):
        team = Team(name=f"Test Team {i}", description=f"Description {i}", is_private=False)
        db_session.add(team)
    db_session.commit()
