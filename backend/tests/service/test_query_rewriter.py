...
class TestQueryRewriter(unittest.TestCase):
    ...
    def test_query_rewriter_with_all_rewriters(self):
        config = {
        ...
        with patch("rag_solution.query_rewriting.query_rewriter.HypotheticalDocumentEmbedding") as mock_hde:
            mock_hde.return_value.rewrite.return_value = (
                "test query AND (relevant OR important OR key) hde rewritten query"
            )
    ...
    def test_query_rewriter_invalid_config(self):
        invalid_configs = [
            "not a dict",
    ...
    def test_query_rewriter_empty_query(self):
