...
def sample_query_result() -> list[QueryResult]:
    ...
    return [
        ...
        QueryResult(
            chunk=DocumentChunk(chunk_id="3", text="General relativity explains gravity as curved spacetime."),
            score=0.7,
...
@pytest.mark.atomic
def test_evaluator_initialization(evaluator: RAGEvaluator) -> None:
...
def test_evaluate_cosine(evaluator: RAGEvaluator, sample_query_result: list[QueryResult]) -> None:
...
def test_edge_cases(evaluator: RAGEvaluator) -> None:
    ...
    # Single document
    single_doc = [QueryResult(chunk=DocumentChunk(chunk_id="1", text="Test"), score=1.0, embeddings=[0.1, 0.2])]
